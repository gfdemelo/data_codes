...
invert=sorted(inverse_dict(lcount), reverse=True)

lista=list()
for item in invert:
	val=inverse_dict(lcount)[item]
	lista.append(val)	

ax.bar(x, [invert[i]/norm*100 for i in range(len(invert))],  width=0.8, color='g', alpha=0.5, align='center')
ax.set_xticks(x)
